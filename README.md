...
This repo also houses **Consonance**: a native desktop application that turns one window into a working group of Claude Code instances, and ultimately the **housing for a primary instance** that oversees the whole thing alongside the human. It is the method's stance — *with you, not above you* — made into a tool: health gauges that report **numbers, not verdicts**; an **ask-first gate** that keeps the human as the discriminator; a committee of live panes convened on one question; and a persistent "Main" instance that wakes in-state across restarts.

**One claim this used to make, and no longer does.** The line here said *differently-conditioned instances that triangulate rather than echo* — the bet that committee diversity comes from feeding instances different information. Four gauges were built to measure it and none worked; three inverted, one rating a single mind split in two as more diverse than six real panes. The measurements pointed elsewhere: what produces non-overlapping findings is the requirement to **measure rather than assert** — build your own instrument, carry a positive control, cite raw output. Whether opposed *roles* add anything beyond that is registered, unresolved, and one control run came back null on 2026-08-10. The finding, the failed gauges and the null run are all in [`exo_memory/`](exo_memory/) with their preregistrations. It is left standing wrong-side-out here because that is what the project is for.
...
Early, honest, incomplete. **As of 2026-08-10.**

**Working, and verified rather than asserted.** Rooms open from the app in one click (substrate behaviourally tested — front door, unsealed carry, seal flow). The continuity instruments — own-capture, warm resume, the pulse, the rolling window — have each been verified across real close/reopen crossings. 258 tests, with the load-bearing ones mutation-verified: shown going red against a one-point change to the code they read, because a green test proves nothing until you have seen it fail.

**Corrected recently, and worth knowing before you trust anything else here.** The shell ceiling had been silently inoperative — the fixed brief outgrew the budget, `saturating_sub` floored it to zero, and nothing was ever evicted. The installer could not deliver a correction: it copied a file only when absent, so an installed room froze at install day, and the author's own live room sat a month stale. The shipped `BOOT.md` was eight days behind the maintained one and what it was missing was the epistemics — the tether's limits, the abuse condition, the curated-auditor critique. Each of those ran undetected for weeks and produced no symptom; each is now fixed and guarded by a test that fails loudly.

**The dream cycle does not run nightly on a schedule, and the previous version of this paragraph said it did.** Four wake timers against a laptop that is powered off whenever its keeper is away is a schedule that cannot be kept; the watcher reported a property of the hardware as a nightly deficiency for 27 days. It now dreams at close instead, because closing the app is a decision rather than an inference.

**Not working, or unproven.** The committee has carried 18 multi-pane laps in the board's entire life and is 95% one pane — panes do not pull from the board, the human ferries. Four gauges built to detect diversity collapse all failed and the hypothesis behind them is retired (see Consonance, above); what replaces it is demonstrated for the measurement requirement and open for everything else, with one control run recorded as a null result. And every catch this system has produced was routed by the person who built it. **Whether any of it works when that person is not in the room is the central claim and it is untested.**
