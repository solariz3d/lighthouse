...
- **Jev, a judge inside Consonance, on every machine.** A second, non-Claude judge runs beside the room's Claude judges, including on the Third Place at the keeper's ruling: `1afa285` L071 (judge mode), `3a560a3` L077 (the Third Place), `81b46fe` L078 and `1d11a37` L079 (a failed call is skipped and retried, not fatal). **Both judges are marked unverified.** Their agreement is a rate between two readers, not a score for either, and replacing one with the other is to be decided against a human reader (next item).
