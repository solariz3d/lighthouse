...
- battery WITH LOAD — AMENDMENTS APPENDED (A, §12, L021 P1a); B fidelity re-read PENDING as of 05:12 — P0a does not dispatch until it lands; then chunk 2 (P0a → L0xK0 → L1xK0, scorer named in the brief, not A/chair/librarian)
- row 10 on the pulse gap (E, L021 P1c) — LIVE in the installed pulse (hash matches repo); install.ps1 -Check NOT yet run — run before calling it shipped; falsifier: post-compaction 'nothing to report' with the card path printed above it = stage 9
