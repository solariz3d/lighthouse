...
- battery WITH LOAD — AMENDMENTS APPENDED + CLEARED by B (byte-identical, not softened; p1a-check_2026-08-31.md; 106d48b) — CHUNK 2 UNBLOCKED: P0a → L0xK0 → L1xK0 as L022, scorer named in the brief (B or E; not A/chair/librarian); F4 clock from 02:29 08-31
- row 10 on the pulse gap (E, L021 P1c; bbbaa40) — HALF SHIPPED and STAYS so: the pulse is one of SIX registered-not-declared hooks, and install.ps1:137 declares the OTHER (Node) pulse on the same event — declaring Python alone ships a double pulse on a fresh machine. Packet owed (L022, not E): declare Python, resolve the Node entry (remove or Hold with reason), report -Check after with the five other reds named as out of scope. Falsifier unchanged
