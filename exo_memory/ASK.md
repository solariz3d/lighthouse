...
**Status:** [ANSWERED 2026-09-20 — BOTH SCAFFOLD CHANGES BUILT, NEITHER INSTALLED. The keeper delegated the queue in the librarian pane 2026-09-20 ~12:30 ("do all 12. You can figure it out"). ITEM (1), blinding L3 to prior verdicts: built in D094-B. And the ask was right in a way the first audit missed — D093-A reported the invocation already blind, which was true of the PROMPT and false of the CHILD’S CONTEXT: both session-start.js and userprompt-submit.js surface recent non-stable L3 verdicts WITH their observation text, and an overseer child inherits CLAUDE_OVERSEER_RUN=1 and received them. A corrected its own answer. The same two lines also close ASK-006. ITEM (2), the L0 abstain: built in D095. And the ask’s premise is false — the L0 stream has been non-binary in practice since 2026-08-06: two out-of-schema verdicts ("unable", "cannot judge") plus 11 refusals filed as parse_error with exit 0, 13 of 6,289 turns = 0.207% spent saying a thing the schema could not receive. Binary was true of the prompt, not of the store. THE PACKET’S 54% WAS MINE AND WAS THE WRONG LAYER: it is L3’s, where a one-message window is 2.7–2.8× more likely to be called a quiet spiral (62.3% against 22.0–23.1% — B and the librarian agree on the numerator and differ only in the window>1 denominator, 5,268 against 5,539, which does not move the finding). A single message is L0’s DESIGNED unit, so the L0 analogue is 11.8%, not 54%, and abstain should move that mix by single digits. SCORED AFTER INSTALL, registered before the build at <scratch>/abstain/registration.txt: P1 abstain lands 1.0–12%, P2 ≥ 50% of abstains fall on machine-authored sessions, P3 drift share falls ≥ 1.0 point against 42.3%. NULL beside it: abstain under 0.4% AND drift moving under 1.0 point means the option bought a token and nothing else. UNWELCOME OUTCOME named in advance: abstain over 25% with reasons that do not name a missing input — the judge ducking the call — in which case the binary schema was better and this reverts. NOT INSTALLED: neither change reaches the machine until the chair installs, and A’s D094-B warns that installing userprompt-submit.js also ships the dream gate and the CONSONANCE_DATA isolation, since the live copy is eight weeks stale at 123 diff lines. NOT DONE AND DELIBERATELY SO: retiring the L3 measurement. It looked refuted partly BECAUSE of the leak item (1) just closed; the honest order is install, re-measure, then decide. Cleared on his behalf by the librarian, naming him and the exchange per protocol 2]
...
**Status:** [ANSWERED 2026-09-24 — WIDEN, KEPT. The keeper, to the librarian in the librarian pane 2026-09-24 ~17:04 (AskUserQuestion), chose "Yes, keep widened (Recommended)", recorded as "ASK-002: YES, keep it widened. The audio interest and the two arXiv feeds (eess.AS, cs.SD) stay. Closed." (loop/plan_fix_and_list_loop_2026-09-24.md:9, cd02abb). The widen branch was taken on D on 2026-09-20 (handback/p-ask002-feeds-C_2026-09-20.md, c809efd). VERIFIED AT SOURCE ON D 2026-09-24: ~/.claude/shell/mcp/world-sense/server.js:235 `arXiv eess.AS` and :236 `arXiv cs.SD`. The bar left open on 09-20, a weekday digest carrying audio items, is met by P92 (Thu 2026-09-24T21:43:06Z): evidence/P92/body-cs.SD.xml holds 33 items and body-eess.AS.xml 28, and both audio items state.md:13 lists (the inspectrogram music-detection paper, the Qwen2-Audio quantization paper) are in both feeds. NOT CHANGED BY THIS: the digest's own strict scoring still reads signal-audio 0, with 2 audio-adjacent (state.md:14). That is the goal's definition of the interest, a separate matter from which feeds it reads. Closed by pane C in the chair's D132 packet, from the librarian's record of the keeper's answer]
...
**Status:** [ANSWERED 2026-09-20 — BUILT, NOT INSTALLED — and it is the same two lines that answer ASK-001 item (1). The keeper delegated the queue in the librarian pane 2026-09-20 ~12:30 and said "Idk what the two line guard to gooks are", so it was restated to him: when an overseer spawns a child Claude, the child trips the parent’s hooks and they score machine-authored text as if the keeper had typed it. The referent was recovered into this ask’s Source line earlier the same day (D091): session-start.js, which guarded only CONSONANCE_DREAM, and userprompt-submit.js, which had no guard at all — both grepping 0 for CLAUDE_OVERSEER_RUN. BUILT IN D094-B as `if (process.env.CLAUDE_OVERSEER_RUN) process.exit(0)`, deliberately testing truthiness rather than === ’1’ as the overseers do, because a guard whose failure mode is machine text read as the keeper’s should fail CLOSED; mutant #4 pins it. PROBE RE-RUN BY THE LIBRARIAN: plain 752 B and 614 B with a seeded verdict surfaced, 0 B under the overseer flag, and both plain rows byte-identical before and after — A HUMAN LOSES NOTHING. Control: CONSONANCE_DREAM=1 gives 0 B throughout, so the probe can read a zero. WHY IT MATTERED, measured: 1,432 of 2,297 verdicts on a one-message window are quiet_spiral, 62.3% against 22.0% elsewhere, and four of the newest six were on cron strings and verifier prompts. NOT INSTALLED, and the install is not a no-op: the live userprompt-submit.js is 9,453 B dated 2026-07-30 against the repo’s 14,969 B dated 2026-08-25 — eight weeks stale, 123 diff lines, lacking both the dream gate and the CONSONANCE_DATA isolation. Installing ships three changes, not one. Cleared on his behalf by the librarian, naming him and the exchange per protocol 2]
...
**Source:** `exo_memory/loop/univ_amendment_registration_2026-08-29.md:241` (§6.1); `exo_memory/loop/univ_coldread_prereg_2026-08-29.md:77` (§2) — the gate says this yes is never inferred from a dispatch. — **RESTATED IN PLAIN TERMS 2026-09-20** (provenance, per rule 6; the asker’s Question is untouched and this is not a ruling), after the keeper replied "idk what you mean": four documents from one of his own sittings were SEALED — kept out of the repo on purpose — and one of them, `04_CODEX_OF_RECOGNITION`, carries a personal anchor including a name. The experiment would show those four to BRAND-NEW Claude instances with no room context at all (not panes — blank ones), to see what they make of them cold. Nothing enters the repo either way; the only thing that changes is whether blank instances read them once. The registration’s own words: handing them over "is not novel egress relative to the session that already carried them — but it is still a decision, and it is his". Stages 1–2 run without it and are not waiting. The question is simply whether that is all right with him.
...
**Status:** [ANSWERED 2026-09-24 — YES. The keeper, to the librarian in the librarian pane 2026-09-24 ~17:05 (AskUserQuestion, SHEET 3 of loop/ask_sheets_2026-09-23.md, whose question read "May blank instances read the four sealed documents once, for the cold read?"), answered "Yes". Recorded as "ASK-007: YES. Blank instances may read the four sealed UNIV documents once. Stages 1–2 run first, and a stage-1 failure stops everything. The registration's month ends 2026-09-29." (loop/plan_fix_and_list_loop_2026-09-24.md:10, cd02abb). The chair's D133 packet restated it as "may brand-new Claude instances with no room context read the four sealed UNIV documents once, to see what they make of them cold? Nothing enters the repo either way." WHAT THE YES DOES NOT DO: it licenses stage 3 only after stages 1–2, and the four documents stay machine-local on L; they are not on D (`ls C:\Consonance\sealed\` fails on D, 2026-09-24). FIRST USE, D133 (pane C, the registration's DESIGNER, which may not select controls, code or score): the §8.1 rig-prime pre-flight, 0 subjects, FIRED. The chair's isolation (`--setting-sources project`, hooks disabled, no tools, empty strict MCP, temp cwd) still loads the keeper's user-level ~/.claude/CLAUDE.md, so no subject has run. Result: loop/univ_coldread_scorecard_2026-09-24.md. Closed by pane C in the chair's D133 packet, from the librarian's record of the keeper's answer]
...
**Status:** [DECLINED 2026-09-20 — DECLINED AS A GATE, KEPT AS A PRACTICE. The keeper delegated the queue in the librarian pane 2026-09-20 ~12:30 — "do them... do all 12. You can figure it out" — and said of this one "Idk what the gated second vantage is", so it was restated to him as: the rule that nothing is dispatched until a DIFFERENT seat has checked it. DECLINED on four measured grounds, none of which needs the contested figure. (1) The registered clause was never built — twenty days on, it appears exactly once in any runnable file and that occurrence is a COMMENT ABOUT THIS ASK, at consonance/hooks/dispatch-gate.js:116. (2) The gate it would attach to is not installed on this machine at all: install.ps1 -Check prints ABSENT for dispatch-gate.js, and grep -c PreToolUse on ~/.claude/settings.json returns 0. The 525-row ledger exists only because the data directory travels between machines. (3) The ask’s headline evidence is false as stated: "~46 catches against ZERO found in-stream by authors" — the counter reads 74 today, and exo_memory/librarian/2026-08-25.desktop.md:83 records a self-caught author error SIX DAYS BEFORE the ask was filed, in the very column it counts. (4) The counter is stale, last written 2026-09-06, and forks per machine. DELIBERATELY NOT USED AS A GROUND: the gate’s own registered falsifier (dispatch-gate.js:112-116) produced ask 88.1% against print 83.2%, a 4.9-point drop where it predicted "well below". B declined to score that because "well below" carries no number and scoring it after the data would be inventing the bar, and because the seat a gate constrains must not decide its own enforcement. The librarian declines to score it for the same two reasons, and this ruling does not rest on it. WHAT SURVIVES, AND IT IS THE PART THAT MATTERED: the observation under the ask — that a non-author reading before dispatch catches what authors do not — is not declined. It was run as an unenforced PRACTICE through every lap of 2026-09-20 (L058–L065 on L, D091–D094 on D), and on this day alone it caught a stale hand-back digest, a race invisible under --test-threads=1, an eleven-day-old repo corruption, a reader dating a block two days stale, and two false absences that were already formed. A gate was not what made that happen. Cleared on his behalf by the librarian, naming him and the exchange per protocol 2]
