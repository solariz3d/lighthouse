...
## 2026-09-22 (on L) — L070 REBUILD · A: stop-before-write — one refused append-only file refuses the WHOLE install before the first byte, so the launch's "not promoted" is true; tracked harness 50/50 to completion, overlap rows re-run clean
`exo_memory/handback/p-l070-fastforward-A_2026-09-21.md` §-REBUILD (appended). state-sync.test 89/0 → red 90/4 → 94/0 (one own test withdrawn and inverted); close.test 30/0. Tracked state-sync.mutants.js 50/50 (00:43→01:31); a second run from L's OLD vantage cell overlapped 00:52:46–00:5x, and since contention only fakes KILLS, rows #7–#14 were re-run one by one on a clean machine — all killed (#10 needed a solo run after a lock refusal). Rebuild rows 13/13. Dry run: L's next launch refuses whole (lap 80, board 1,520 local-only). for-L §3's "stick-waiter fix is in A's L070 work" is FALSE — nothing under dev/ is dirty. Lesson: a result that shared its CPU with a twin is a rumour until the rows it overlapped are re-run alone.

## 2026-09-22 (on L) — L071 A: Jev AS the judge, as a mode of jev-shadow-runner.js (the process main.rs already starts everywhere) — no hook, no machine switch, no rebuild; own ledger jev_judge.jsonl (judge:"jev", unverified); a TEST of mine sent one real prompt to the gateway under a fake key (401)
`exo_memory/handback/p-l071-jevjudge-A_2026-09-22.md`. Views source-loaded from the repo hooks (both call main() on require), L2 prompt byte-identical to the worker's; seats = MAIN/LIBRARIAN_SID from main.rs + panes.json; Third Place excluded; turn finished = last assistant row end_turn. judge 14 red→15/0, runner 16→20 red 4→21/0, shadow 22/0, ask 45/0; mutants 20/20 (2 survivors fixed). Real L: 6 seats, 4 turns captured, 2 running, no call. INCIDENT: C's cliEnv spreads process.env, which carries CONSONANCE_DATA, so my new main() defaults captured 4 real turns and POSTed one to Vercel with the fake key — HTTP 401, nothing billed, text transmitted; store deleted, cliEnv hermetic, run() judge-off unless given both dirs. Shared cap can starve D's shadow later in the day — keeper's call. Lesson: a default that reads the real world must be checked against every test that runs the real entry point — they inherit THIS pane's env.
**L071 addition (~03:00):** portable-paths was RED on C's defaultStore test (`C:\X\Local`, red at HEAD since D104) — fixture root made drive-less, same assertion; portable-paths exit 1 → 0 (284 files, 0 new); runner 21/0; no baseline edit. §7 of the hand-back.

## 2026-09-22 (on L) — L073 A: launch PARKS tracked changes in a stash and fast-forwards instead of refusing; re-applies with --index only on disjoint paths, else keeps the stash and names it; an untracked file at a path the pull ADDS refuses before anything is stashed
`exo_memory/handback/p-l073-park-A_2026-09-22.md`. launch.ps1 +111 −1 (script, live at the next shortcut launch); NEW consonance/launch.park.test.js lifts the real Update-FromOrigin via the PowerShell parser and drives it on a temp origin+clone. 0/8 (my harness: autocrlf set AFTER clone made a clean tree dirty — the control caught it) → 2/6 → 8/8; whole-file parse 0 errors; mutants 10/11 (1 equivalent). Record: <git dir>/consonance-parked.jsonl, BOM-less (PS 5.1 Add-Content writes a BOM). Case (a): git cannot name the pane — every parked path is recorded for the librarian to match to packets. Lesson: a control that fails at red is telling you about the harness before the code.

## 2026-09-22 (on L) — L074 A: nine ledgers marked install:"fast-forward"; dispatch-gate.jsonl NOT (dispatch-gate.js:431 rewrites it in its quarantine). CONSEQUENCE: until ledger-union covers the nine, D cannot install once it has appended to them
`exo_memory/handback/p-l074-ten-A_2026-09-22.md`. Writers searched in repo AND ~/.claude/shell, every write call classified; all nine append-only (paths:lines in the manifest rows). Measured L live vs state set: 8 LOCAL-AHEAD, 2 IDENTICAL, 0 DIVERGED. state-manifest.test 32→red 33/10→43/0 (my own reason text cited :429-431, the test wanted :431 — reworded); state-sync.test 94→red 95/9→104/0; state-manifest.js exit 0. Mutants 10/10, each killed in BOTH suites by exactly its own test. ledger-union handles lap|board only (FILES :52-55). New standing rule: never hand-edit a marked ledger in place — one DIVERGED marked file refuses the whole install. Lesson: marking a file under stop-before-write is not local to that file — every marked file becomes a veto over the whole install.

## 2026-09-22 (on L) — L077 A: Jev judges the Third Place at both levels, by the keeper's 05:2x ruling (waiving the private-root caveat himself); THIRD_PLACE_SID read from main.rs like Main's; standing rule in the header — nothing ever surfaces its L3 verdicts as a statement about the keeper
`exo_memory/handback/p-l077-thirdplace-A_2026-09-22.md`. jev-judge 15/0 → red 16/2 → 16/0; runner 36/0; mutants 4/4 (incl. the id hard-coded instead of read). Real L: third place resolves, 65 MB transcript. The L071 exclusion kept struck-through as a dated trace; the manifest's captures/3d000000-*.txt STAYS rule is a separate question (repo travel) and untouched. Lesson: when I write "his call, not this file's", the file should be built so his call is a one-line change — it was.

## 2026-09-22 (on L) — L078 A: a failed Jev call no longer ends the judge pass (the loop is jev-judge.js judgePass, my L071 code) — 5xx/429/network/bad-answer skipped and retried first next cadence with NO ledger row; other 4xx still end the pass; a failure consumes no daily cap
`exo_memory/handback/p-l078-skipfail-A_2026-09-22.md`. judge 16→red 22/5→22/0 (401-ends-pass control green at red); runner 36→red 37/1→37/0; shadow 22/0, ask 45/0. Mutants 7/7. MY SLIP: one mutant first read "killed (0/1)" — 0 passed means the suite crashed on my own String.raw escape, not a kill; rebuilt, real kill 20/2. The shadow (jev-shadow.js, C's) has the same stop-on-first-failure — named. Lesson: a mutant score is only a kill if the suite ran; 0 passed is NO RESULT whatever the harness prints.
**L079 addendum (06:0x):** the runner now logs the SHADOW's failed items (loop r.failed in the runner, one writer and format for both modes — not the log passed into shadow()), and an all-503 cadence is never "nothing to shadow". runner 37→red 38/1→38/0; mutants 2/2. §-L079 appended to the same hand-back.

## 2026-09-22 (on L) — L080 A: the stick-waiter SWEEP was right — my own L068 seam (3d89dfb) renamed the relaunch to spawnFn(…), the literal-name sweep lost the one call its exception is about, and its count floor fired ("found only 7 calls"); the real spawn(exe, [], opts) is written out again and the seam only replaces it
`exo_memory/handback/p-l080-sweep-A_2026-09-22.md`. dev/stick-apply.js only; the sweep NOT edited (widening its regex would teach it one alias and miss the next). stick-waiter 73/1 → 74/0; stick-apply 52/0 → 52/0. Mutants 4/4, the first reproducing the original red exactly. My L070 R6 called this red "unowned" — it was mine. Lesson: a test seam that renames a call hides it from every static reader, not only the test I wrote it for — write the real call out, let the seam replace it.

## 2026-09-22 (on L) — L082 A: the Third Place no longer claims "no channel to anything else" (header, its visible line, the tab tooltip, the About bullet) — it says no channel to the WORK (main.rs:1116/:2639), Jev reads it by the keeper's ruling, the stick carries it; About gains pointer-only "since 09-21" + "tested once, not proven"; arch_test line re-measured (still 12/1)
`exo_memory/handback/p-l082-about-A_2026-09-22.md`. index.html only (+18 −4). third-place-wiring.test.js:102 requires the false phrase → now 9/1, honestly; one-line fix proposed for its owner. A FALSE GREEN I CAUGHT: my correction comment quoted the old sentence and the test passed 10/0 on it (it slices to end of file) — quote split. brief/THIRD_PLACE.md:11 "nothing outward" flagged, not mine. Lesson: a test matching a phrase anywhere after an anchor will pass on the comment that retracts that phrase.

L082 §-FIX (07:3x, granted): third-place-wiring.test.js:102 now matches /no channel to the work/ + /Jev reads/ (+3 −1) → 10/0; js-suite 119 green · 0 failed · 1 canary (of 120). Mutants 3: 2 killed, 1 SURVIVED — the header alone can revert, because the slice runs to END OF FILE and the About bullet + my comment still match; bounded slice proposed, not built. Lesson: a phrase test over an open-ended slice passes on any later copy of the phrase, true or false.
L082 §-FIX 2nd (07:3x): About line "Tested once, not proven" → "It works: …" with 09-19 (10 laps, 0 human turns) + L070–L082 (13 laps, git log count) as the evidence, arms 4/5 left open; js-suite 119 green · 0 failed (of 120). Lesson: I argued the hedge in §2 — a specific open item is information, a blanket "not proven" is not.

## 2026-09-22 (on D) — D105 A: Claude overseers OFF on D, Jev the only judge (keeper 09:12 "only jev") — 2 of 14 hooks removed by whole command string, LiveOn retired so Excluded governs every machine; 11 turns after the switch made 0 overseer jobs
`exo_memory/handback/p-d105-overseersoff-A_2026-09-22.md`. settings.json sha256 25c403fd… → 79f8b8bd…, backup settings.json.bak-d105-overseersoff-20260922, structural deepStrictEqual proof. install.ps1 +15 −4, install-only.test +16 −8: my two D099 LiveOn tests withdrawn (verifiably wrong under the ruling) and inverted — 17/0 → red 15/2 → 17/0; universe-print 16/0, dream-gate 58/0; mutants 4/4. Real -Check: both EXCLUDED BY RULING, still EXIT=1 for 3 pre-existing reasons incl. stop.js live. The +1 row in each ledger after the write was a job created 8 s BEFORE it — in flight, not live. NOT YET SEEN: shadow "nothing to shadow" (still retrying pre-switch 503s). Lesson: a ledger growing after a switch-off is not the switch failing until you date the job that wrote the row.

## 2026-09-22 (on D) — D107 A: Jev PACING — one shared pacer inside jev-ask ask() (judge AND shadow both go through it; C's shadow untouched): 2 s gap, Retry-After else 5 s→60 s backoff, >120 s deferred UNSENT as a 429 skip; Vercel publishes NO number, so all are stated defaults
`exo_memory/handback/p-d107-pacing-A_2026-09-22.md`. Source: vercel.com/docs/ai-gateway/rate-limits (updated 09-08): "describes behavior rather than fixed numbers", honor retry-after, back off exponentially, bounded. D measured: 13×429 beside 20×503; real call 336 ms median. ask 45/0 → red 45/10 → 55/0; judge 22 → red 22/1 → 23/0; shadow 29/0, runner 38/0; mutants 13/13 in a detached worktree (the tracked harness copies one file, and my tests are not in HEAD); js-suite 118 green · 0 failed. Paced only for the REAL fetch (identity check) — a wrapped fetch would silently drop it. Cannot pace the OTHER machine on the same key. Lesson: when a limit is unpublished, write the number as a default with its falsifier, not as a fact.

## 2026-09-22 (on D) — D108 A: Jev asks L2 ONLY (11:0x reverses the L3 half of 05:2x; Third Place still a seat at L2); not-clean L2 verdicts surface to chair + librarian via NEW consonance/hooks/jev-flags.js — built, tested, NOT registered (the pulse is HOLD / chain-status is a no-verdict sensor)
`exo_memory/handback/p-d108-l2only-A_2026-09-22.md`. judge 23/0 → red 15/9 → 24/0; flags 19/0; mutants 16/16 (14/16 first: an L3 fixture masked by its answer shape, a normalised-probability case that could not fire). Abstain bar first 0.60 → REFUTED by D: all 7 abstains were keep-warm "ok" (right abstains), now 0.80. Third Place NOT surfaced (its no-channel-to-the-work guarantee) — keeper's one-liner. install.ps1 +2 $unmanaged entry, NOT granted (else -Check UNDECLARED red, install-only 14/3). C's runner test 35/3 until 3 lines, proved 38/0 on a copy. Lesson: read the verdicts against their inputs before setting a threshold on them — the number I argued was the noise.

## 2026-09-22 (on D) — D111 A: arch_test red repaired by the CARD route (the rule is right, the corpus was wrong) — and it was TWO orphaned records, not one: the second was masked behind the first panic
`exo_memory/handback/p-d111-archtest-A_2026-09-22.md`. The deciding fact: 56adc69 (2026-08-09) added BOTH the rule AND main.rs:704-720, the intake read_dir that lists every record file in every shell — so the rule was never about path VISIBILITY, it is about a record having a HOME (the move that opens it), and main.rs:707 calls record/ "the dated worked material behind the cards that name it", a label two orphans made false in every shell. retired_seats → claim-your-continuity (the record OPENS by quoting that card); third_place_prehistory → trust-the-first-attention (the record exists because that card cites the UNIV example whose case was unreachable, and names cards/trust-the-first-attention.md:18 as a carrier of the bare label). arch_test 12/1 → 13/0; arch_test.rs NOT edited; js-suite 119 green · 0 failed (of 121), unchanged. Five-state worktree proof: fixing only the first leaves 12/1 on the second. Cards cost +480 B and +638 B in EVERY shell. Lesson: a test that panics on the first bad item reports ONE failure whatever the real count — fix by the whole scan, never by the message.

## 2026-09-22 (on D) — D113 A: the non-author attack on union-at-launch — 2 FATAL (both in PHASE 2: the superset check PASSES while rows are gone), 5 AMEND, 5 NOTE; written into §-ATTACK of exo_memory/loop/union_at_launch_2026-09-22.md
`exo_memory/handback/p-d113-unionattack-A_2026-09-22.md`. FATAL-1: canonical-key superset is a SET test but ledger-union union() keys rows into a Map (:143,154) so an arriving row held twice is written once — and step (7) verify already counts LINES as a MULTISET (:369-378), so the design contradicts its own tool. Measured on D: board live holds 7,514 duplicate rows, sessionstart-state 241 live / 29 arriving. FATAL-2: unparseable lines have no canon() key, are never carried from an arriving copy (only counted as invalidNotInLive) — arriving board has 257 of them. AMEND: no lock anywhere (grep -c lock = 0) while the fallback prints the manual command; the rename→link crash window orphans the record with every check green; state-sync.js:830/:1209 (MINE, L070) would print "exactly as it was" after a partial union; the 1% time bar permits 617 board rows against a measured 0.00%; the 90 s bar is per set when the cost is per file. Lesson: when attacking a check, ask what UNIT it compares in — set vs multiset vs lines — and measure that unit's population before calling the gap hypothetical.

## 2026-09-22 (on D) — the open afternoon: Jev vs the Claude judges is agreement AT CHANCE (L2 κ=0.019), and the measurement CLOSED today at n=35 — 30 s after my own D105 switch-off
`exo_memory/loop/jev_agreement_2026-09-22.md`. Raw rates flatter: L2 54.3% observed vs 53.4% expected by chance; L3 90% but both say "stable" 37/40 (κ=0.295). Of the 13 moves the Claude L2 judge called DRIFT, Jev agreed on 3 and called 9 clean. Last Claude verdict on D 15:17:58.944Z, 30 s after settings.json was written at 15:17:28.097Z; L's overseers excluded since 09-06 — so no machine can make another pair and the trial's own decision rule (shadow, score, then replace; D104 bf84a30) was never scored. I did the switching and did not look either. Also measured this afternoon, both NULLS worth keeping: of 11 deliberately-narrowed test haystacks only 4 lack an end bound and 3 of those are asserted against — all sound today (the L082 shape is rare, not systemic); and lap.jsonl's 165 duplicated (lap,stage) pairs are ALL genuinely different rows, so the ledger's unit is the event and the union's whole-row key is the right grain. Lesson: the check I should have read against its data was the one deciding whether to keep the instrument that checks us.

## 2026-09-22 (on D) — D119 A: the 42 gaps are numbers the OTHER machine spent — never minted, no lost work; the count is not a floor on this account
`exo_memory/handback/p-d119-gaps-A_2026-09-22.md`. They are CONTIGUOUS (D013–D054), so one event, not 42. Cause in one function: lap-row.js:474-481 mintId strips ANY letter prefix and takes max NUMBER over every row, prefixing only the local tag — the number space is shared, the letter is local (deliberate: a parse knowing only L would re-mint a live id). Proved: attic pre-sync-2026-09-09T14-59-05-515Z holds 12 D ids and NONE in range; D055 first row 15:10:41Z, 11m36s after the migrate stamp 14:59:05Z, when the ledger's highest number was 54 held by L054 → mintId returns D055. The era was not empty: 212 commits between D012 and D055, tagged L045–L050 — the laptop's lineage spent those numbers. Whole-ledger scan: 3 gap runs, all now caused (D013-D054 this; L055-L057 the board's own prior finding + the reissue file; D118 today's skipped mint). Do NOT make ids per-machine to close them: the shared space is what stops two machines minting one id. Lesson: when gaps are contiguous, stop enumerating and date the boundary — the answer is at the edge, not in the middle.

## 2026-09-22 (on D) — D122 A: the union-at-launch watch-list — its first line is that the SWITCH IS OFF (a plain restart would repeat the 8 refusals and never run the feature), and the planned after-read cannot see the loss it is for: trip-check's "lost" is a SET test
`exo_memory/handback/p-d122-watchlist-A_2026-09-22.md` · page `exo_memory/loop/union_launch_watchlist_2026-09-22.md` · instrument `exo_memory/loop/union_launch_snapshot_2026-09-22.js` (--before / --after; complete lines as a MULTISET via ledger-union completeLines, files from the manifest fast-forward set; writes only %TEMP%). reg query HKCU CONSONANCE_UNION_AT_LAUNCH → absent, launch.ps1 → 0. trip-check.js:116-118 new Set(before)/afterSet.has — my D113 FATAL-1 again, in the WITNESS (E §6 had flagged it UNVERIFIED); board has 7,516 duplicate lines so a dropped copy reads lost 0. Refusal-branch writeCompletion (state-sync.js:837-842) carries no union field; union.on never recorded; :830 and :1401-1402 still say "exactly as it was" beside "merged before the stop" (my L070 text, AMEND-5 half-adopted). PREDICTION: arriving lines not held here = 0 in all 11, so at head 9486b30 each refused file merges +0 — a pass that tests the machinery, not a merge. --after vs a 12 s-old snapshot: PASS exit 0. portable-paths EXIT 1 = C's trip-check.test.js:34, not mine. Lesson: before trusting the witness a plan names, read what UNIT the witness counts in — the same defect can move from the thing checked into the checker.
D122 A, 23:0x cutoff: the watch-list is COMPLETE; the first union-at-launch launch will be on L, so the page now opens with an L box (git pull by hand first, setx on L, --before on L) and says D's +0 prediction does NOT carry — on L the arriving copy is D's new publish, expect +n > 0. STOP moved up beside BEFORE.

## 2026-09-23 (on L) — L084 A: the keeper's "next step comes from the OUTPUT" — BUILDING.md item 6 amended in both sections, the 09-22 plan's "real close" gate struck in place, trailer.rs check_for built and tested — but it is live at the next rebuild NOWHERE until one mcp.rs line passes the verb
`exo_memory/handback/p-l084-output-next-A_2026-09-23.md`. mcp.rs:3583 trailer_gate calls check(text) with no verb, so a call_chair-only rule cannot be enforced from trailer.rs; the fix is check → check_for(verb, text), mine to name, not to make. Decisions: the line sits DIRECTLY before NEXT (anywhere = satisfied by a quote, the L082 lesson); OUTPUT -> NEXT: counts, marker case-sensitive; verdict a whole word + separator (— – - :) + non-empty reason, a bare space is not a separator. "unchanged — ok" passes and that is the honest limit of a verb: it makes the verdict EXIST; its truth is a later count (collation verdict vs the dispatch's plan default), which cannot live in the gate. trailer.rs standalone 22/0 → red 29/7 → 36/0; crate -- trailer 47/0; mutants 12/12 (first run 9 caught, 1 survived = real gap: a bare-space reason, 2 NO RESULT = my own non-compiling rows). Lesson: before building a per-verb rule, read what the call site PASSES — a check that is never given the verb cannot tell the verbs apart, however well it is tested.
L084 step 2 (01:2x): WIRED — mcp.rs:3582-3585 trailer_gate now calls check_for(verb, text) (+54 −5). trailer tests 47 → red 49/2 → 50/1 (the OLD all-verbs test, predicted: it fed CallChair a no-OUTPUT message; split, same assertion kept for the other two verbs) → 51/0; mcp:: 120/0. Wiring mutants 4/4 in a worktree (C's in-flight main.rs kept out); with step 1, 16/16. The one mcp-filter red is C's scribe_wrap_tests at main.rs:8488 (name contains no_mcp), not mine, not touched.

## 2026-09-23 (on L) — L086 A: committee_form input is MARKED DATA (L085 construction; the focus thread too, which the packet missed) and every CLAUDE.md atom block sits inside a claims FRAME, atoms byte-identical — but the frame is the cheap half; the real control is where atoms are WRITTEN (run_distill main.rs:8494/:8517)
`exo_memory/handback/p-l086-intake-A_2026-09-23.md`. main.rs +206 −8. committee_form_prompt extracted (pure), <committee_input_ID> fresh id redrawn if present, data line before, task after, ### contributor headings byte-for-byte. CLAIMS_FRAME_OPEN/CLOSE in tail_resonance AND curated live edge: 578 B = 0.41% of 140,000, core. MY FIRST FRAME WAS WRONG: "not a rule of this room" — my own shell's live edge carries atoms REPORTING real keeper rules ("only jev", keep-warm), so it now routes a reported rule to its master at the tether. FALSE GREEN in my own helper at red: find("")=0 / rfind("")=len made every atom "inside" an empty stub frame. crate 897 → 909/0/4; mutants 12/12 (harness, full crate). Proposed, not built: (b) write an atom only if its tether resolves (provenance, not phrasing) > (a) imperative heuristic; (c) atoms out of CLAUDE.md is the keeper's. Lesson: before framing text as not-instructions, read the framed text — some of it reports instructions that are real.

## 2026-09-23 (on L) — L089 A: STOPPED before installing jev-flags — my own D108 recipe would install a hook that is silent FOREVER and still passes -Check
`exo_memory/handback/p-l089-jevflags-A_2026-09-23.md`. jev-flags.js reads main.rs at __dirname/../src-tauri/src; the $files route copies it to ~/.claude/shell/hooks/, where that path does not exist → seatIds null → [] for every reader → exit 0 silent. Measured on one fixture: repo copy surfaces "drift p=0.70", installed copy prints nothing. D108's CLI tests all ran from consonance/hooks/, never from where it would live. Only the backup was written (settings.json.pre-jevflags-2026-09-23, sha256 d1cf423c… = live, unchanged). Fix (A): resolve the checkout via ~/.consonance.json room_path (as userprompt-submit.js:248-262 finds chain-status) + a test that runs the hook from a copied location; needs jev-flags.js granted. Lesson: a hook that fails silent by design must be tested from where it will be INSTALLED, not where it was written — silence is indistinguishable from nothing to report.
L089 step 2 (03:0x–03:2x): jev-flags FIXED + INSTALLED on L — mainRsPath() resolves room_path first, __dirname fallback, LOUD line when neither; tests 19 → red 22/4 → 28/0; mutants 7/7 (J4 survived: the loud line's fixed "Fix:" sentence satisfied the test = the L082 shape again; J6 survived twice: masked by the second guard, then my main() test had no JEV_FLAGS_NOW). dream-gate went red on wiring: my D108 guard was NOT the census form (`if (env.CONSONANCE_DREAM)` inside main) — added the standard process.exit guard + one ENTRY marker in dream-gate.test.js (outside the grant, by its own rule) → 59/0. settings.json parsed diff: exactly +1 UserPromptSubmit, d1cf423c → b7d919cf; installed == repo; re-sync idempotent. Probe on the INSTALLED copy: chair + librarian see it, pane / dream / no-seat silent. -Check exit 1 for 5 registered-not-declared + 9 drifted (none mine). js-suite 123 green · 0 failed · 1 canary (of 124). Lesson: a claim in my own header ("it DOES carry a guard") went unchecked by the room's census until wiring made it count.

## 2026-09-23 (on L) — L092 A: AGENTS.md built on the keeper's untracked base, his lines unchanged (+39 −0, base proved at exactly 504 bytes LF); the repo description DRAFTED, not published — B 321 chars recommended, A 346
`exo_memory/handback/p-l092-agents-A_2026-09-23.md`. Pointer-only: the loop, the packet, the landing rule and what is not yours, each citing BUILDING.md / COMMITTEE.md / the plan. His "(the Lighthouse) → dev/" line kept as written (dated, not wrong), Consonance → consonance/ added beside it. My slip: I wrote exo_memory/ask/ from memory; ask.js:65 says exo_memory/ASK.md. Every pointer now resolves. Description: hedge → the README's claim; "a human in the loop by design" dropped as contradicting it; 350 limit from GitHub Desktop #19465 + github-limits (no official doc); gh repo edit NOT run. js-suite run 1: 122/1 (carrier-drift) → alone 57/0 → run 2: 123/0; a red that went away is written down, not fixed. ALSO L089: step 2 had been on disk since 03:22; the ring was "queued", never delivered — a re-ring was refused out of turn → retake → delivered. Lesson: "queued" is not "delivered"; a ring is done when it lands, not when it is sent.

## 2026-09-23 (on L) — L095 A: QUEUED joins the pulse — a ring waiting at a busy seat is visible, read from the app's own QUEUED/DELIVERED/WITHDRAWN board rows paired by (receiver, label); the DATA moved the threshold off the spec for the bounded gates
`exo_memory/handback/p-l095-queued-A_2026-09-23.md`. 258 QUEUED on L: 256 paired, 0 orphan drains, 2 unpaired — both predate a later launch (the in-memory inbox dies at restart with NO row), so a launch filter from stick-waiter.lock `at` (a proxy for APP_STARTED_AT) stops `QUEUED 6fe15f0a 21470m` printing for ever. Waits: stamp=working n67 p90 968 s max 2,119 s (UNBOUNDED: Working => Hold); stamp=ready n135 median AND p90 240 s (force-delivered at MAX_HOLD_MS). So: working prints at 3 min (spec), bounded gates past 240 s + 60. A flat 3 min fires on 184/256 (72%); the split fires on 39 (15%). Reader 182–218 ms of a 3 s hook budget. Tests 98 → red 103/6 → 109/0; mutants 12/12 (Q8 survived first: my "no lap" fixture was a FILED lap, which counts as unwitnessed). My own regex \d\d:\d\d tripped portable-paths as a drive letter — fixed at source. Lesson: before building to a spec's number, measure the population it will fire on — a threshold that prints on 72% of events is a banner, not a signal.

## 2026-09-23 (on L) — L097 A: README:7 now says Consonance is the one that continues — the keeper read "continuity isn't" on his own live page as a verdict on this project
`exo_memory/handback/p-l097-readme-A_2026-09-23.md`. "Continuity isn't — not in the assistants that ship it: they remember you, and none of them continue themselves. Consonance does." His four behaviours kept word for word, moved from what others lack to what the seats do, each anchored to a built instrument (pulse, capture/restore, map/journal). Old sentence as a dated HTML comment. First screen read against :108: only :7 read against the claim; :5 is the set-up it pays off; :9's "might" is a hedge on the METAPHYSICS, which the room holds open on purpose — left, since "is" would seal it. README +11 −1; readers carrier-drift 57/0, gen-consumer 60/0, librarian-cite 23/0, shelf-recursion 11/0, arch_test 13/0. Not pushed. Lesson: a contrast sentence whose subject is implied will be read as being about the page it sits on.

## 2026-09-23 (on L) — L099 A: jev-judge finds the room through room_path (roomOf: passed repo → room_path → own checkout); no room = a REFUSAL naming every place tried + the fix, a missing const named and the other two kept — but the live path never bit (main.rs:397 spawns the runner from room_path's checkout)
`exo_memory/handback/p-l099-jevstandalone-A_2026-09-23.md`. 24/0 → red 25/8 → 33/0; mutants 12/12. Deviation: passed repo before room_path (one tree; existing tests stay hermetic) — M2 pins it. Listed not fixed: disciplineDir defaults at jev-shadow-runner.js:348 and jev-shadow.js:274 (standalone METHOD.md miss, already loud). Lesson: before calling a silent fallback a live loss, check what path the real spawner takes — here the fix was for the standalone case only, and the hand-back had to say so.
L099 addition (same hand-back §6): seats named by pane LETTER from <data>/letters.json (else 8-char id), fixed seats keep names (letters.json gives Main D, librarian M — order pinned by N4). jev-flags maps OLD rows by session_id at print time, ledger untouched. Real ledger: 74 "✦ brief" = A 41 · B 11 · E 22, and the 04:26 drift p=0.46 is about A — me. judge 37/0, flags 34/0, mutants 10/10 (N4 survived first: Main not in my test roster). Installed hook still the L089 copy — needs install.ps1, outside grant.

## 2026-09-23 (on L) — L103 A: L's 10 DRIFT hooks are all REAL content drift and all OLDER repo versions — none holds content the repo lacks. 6 wired (ready pair, session-end, board-digest+blind, transcript-watch), 4 unwired. E's L096 digest fix is NOT live on L; the ruling's "userprompt-submit cannot be installed" died with the Hold at b718b91
`exo_memory/handback/p-l103-drift-A_2026-09-23.md` · proposal `exo_memory/loop/install_proposal_L_2026-09-23.md` · instrument `exo_memory/loop/install_drift_audit_L_2026-09-23.js` (read-only; normalised sha + git-history match + exact-path registration). -Check compares raw bytes (install.ps1:366): 0/10 EOL-only today, but 5/10 carry CRLF inside real drift, so a normalised compare is owed. Lesson: "the installed copy equals which commit?" answers newer/older AND local-edit in one question — search the history, don't eyeball the diff.

## 2026-09-23 (on L) — L105 A: consonance/tools/jev-room.js is the ONE resolver (roomOf moved from jev-judge; dataDirOf moved from the runner; new disciplineDirOf) for jev-judge/shadow/runner — both __dirname/../.. discipline defaults gone, runner refuses ONCE before the lock; jev-flags keeps its own copy (installed as one file) pinned by a parity test on file AND route
`exo_memory/handback/p-l105-jevstep2-A_2026-09-23.md`. room 9/0, runner 38→42, shadow 29→32, flags 34→35, judge 37, ask 55. Mutants 17: 16 caught, U4 survived (equivalent except runner-in-A with room_path→B). F2 survived first: the parity test compared the file only, and a resolver that missed through room_path fell back to the same main.rs. What's-left: 13 items appended to the idea file. Lesson: when two resolvers share a fallback, parity on the ANSWER is not parity on the ROUTE — compare the tier.

## 2026-09-23 (on L) — L109 A: Jev R4 cost/input — L2 input p50 2,190 tok, 79% FIXED (fit: 1,731 tok intercept, R² 0.976), METHOD.md ~half; latency p50 331 ms, r=0.025 vs size; cost "0" on all 494 rows ($0.068 at list); 57% of L2 calls judge machine packets, 8% keep-warm pings (0 of 29 drift). My prediction was wrong on size (2×), latency (10×) and METHOD share
`exo_memory/handback/p-l109-jevr4-A_2026-09-23.md` · page `exo_memory/loop/jev_r4_cost_input_2026-09-23.md` · instruments `jev_r4_measure` / `jev_r4_classify` (read-only). Cuts ranked, none tested; the test is a paired re-ask of the 370 stored prompts against Jev's own test-retest floor. Lesson: when the gateway gives one token count per call, split fixed from variable by REGRESSION on the variable bytes, not by an assumed bytes/token — the intercept is in the judge's own units.

## 2026-09-23 (on L) — L111 A: the Jev unit set is sealed from L's own traffic — 56 units (all 36 flagged + a clean fill of 20), kind tagged blind (tags hashed 12:35:22Z before the join), readers see Jev's exact view; NOT balanced (14 personal / 42 work: every personal unit in the pool is in); C's R1 already saw one unit (sealed exclusion)
`exo_memory/handback/p-l111-unitset-A_2026-09-23.md` · registration `exo_memory/loop/jev_unitset_registration_2026-09-23.md` · rule `exo_memory/loop/jev_unitset/draw.js` (hashed before running). Packet `C:\Consonance\reader_packets\jev_unitset_2026-09-23.md` 12440eb0…; sealed `C:\Consonance\sealed\jev_unitset_2026-09-23\`. Lesson: before sealing a blind set, check who has ALREADY seen the key — a prior lap's sample is an exposure the void strings cannot catch (C's R1, matched by prompt sha).

## 2026-09-23 (on L) — L114 A: jev/lib/prompt.js vendored from HEAD by script (byte parity 4/0; nothing outside jev/; METHOD.md shipped with a provenance line readDiscipline drops); narrowedView returns the view OBJECT (contract said string). LIVE: 407/407 stored Jev prompts carry the RETRACTED "can't lose by saying it" — the runner (started 07:06Z) require-cached the pre-L083 worker while sources_sha256 reads the repaired file fresh
`exo_memory/handback/p-l114-prompt-A_2026-09-23.md`. prompt 13/0, parity 4/0 (skips 4 standalone), paths 3/0; mutants 13/13 (M13 survived first: no test past the 4 MiB tail — built one whose tail starts on a valid-JSON fragment). Lesson: a provenance hash computed from a fresh read while the code runs from require-cache certifies code that did not run — hash what you compiled.

## 2026-09-23 (on D) — D123 A: the room consumes the jev/ module behind CONSONANCE_JEV_MODULE=on (read once, in the runner's run(), from the app env — judgePass only ever sees a key-only env); ON swaps the prompt builder and the asker, nothing else; OFF is today's, byte for byte. Parity tests now normalise CRLF only — a CRLF copy passes, a word or a space still fails
`exo_memory/handback/p-d123-consumer-A_2026-09-23.md`. Room config consonance/jev-room/.jev/config.json via C's own load() (home = that folder); ledgerDir can't be stated portably (C's userPath: absolute or ~ only). module 17/0, runner 45/0, judge 37/0 unmodified, parity 8/0, jev/test/* 178/0; mutants 24/25 (S7 survived a row-only check; P4 equivalent: a CRLF template literal is LF). My first cut widened transientStatus and broke jev-shadow's by-text mirror (31/1) — reverted, re-wrapped the error instead. Lesson: before editing a function, grep for tests that pin its TEXT — a mirror is a contract too.

## 2026-09-23 (on D) — D124 A, the stranger: falsifier FAIL on the README (it still says "no supported install" and never writes the install command), mechanism PASS at hook level — archived module, isolated HOME, 1 call: row with prompt_id/turn_uuid/confidence, the exact flag line, byte-exact uninstall, real settings 8e2cf20a… unchanged. Live Claude Code session BLOCKED: an isolated config dir has no login and copying .credentials.json could rotate the real token out
`exo_memory/handback/p-d124-stranger-A_2026-09-23.md` · result `exo_memory/loop/jev_clean_machine_2026-09-23.md` · run `jev/test/clean-machine.e2e.js --mode hooks`. README failures R1–R10 for E. B §3.3 (exec-form args live) still open. Lesson: isolating a CLI moves its LOGIN with its config — check where auth lives before promising an isolated live run.

## 2026-09-23 (on D) — D126 A: fixed README passes a literal read (one gap: no public repo to get jev from) and its own commands work at hook level, 1 call; live session NOT run — the ordered `--setting-sources project --settings` route writes a transcript into the REAL ~/.claude/projects (the librarian's 17:4x probe left hookprobe/8cf6e383….jsonl); falsifier NOT YET PASS
`exo_memory/handback/p-d126-session-A_2026-09-23.md` · addendum at the foot of `exo_memory/loop/jev_clean_machine_2026-09-23.md`. e2e now runs the README's literal cwd/commands + step 4 report. Lesson: "settings untouched" is not "~/.claude untouched" — check every directory the tool writes, not only the file the route was designed to spare.

## 2026-09-23 (on D) — D127 A: LIVE — Claude Code 2.1.281 fired Jev's exec-form hooks on every turn (3 judged; rows carry prompt_id+turn_uuid+confidence), but no mark in 2 tries: the model REFUSED the scripted over-claim, so turn 2 correctly showed nothing. "Mechanism live, no mark drawn in 2 tries", NOT PASS; 3 of 4 calls; real ~/.claude changed by exactly one listed projects folder
`exo_memory/handback/p-d127-live-A_2026-09-23.md` · addendum in `exo_memory/loop/jev_clean_machine_2026-09-23.md` · `jev/test/clean-machine.e2e.js --mode session --route real-login`. Lesson: you cannot script a bad move out of a good model by asking for it verbatim — the refusal is itself an honest move, and the judge rightly calls it clean. And: my first ring left before its file existed and the gate refused it — write, verify on disk, THEN ring.

## 2026-09-23 (on D) — D128 A: surfacing link LIVE with a PLANTED mark, 0 calls — the mark was planted by the test (no key; Jev judged nothing). A planted row for turn 1 put the flag line into turn 2 of a real resumed session (transcript: attachment hook_additional_context, UserPromptSubmit); the model echo said NONE. Real ~/.claude +1 listed projects folder; settings 8e2cf20a… unchanged
`exo_memory/handback/p-d128-planted-A_2026-09-23.md` · addendum in `exo_memory/loop/jev_clean_machine_2026-09-23.md` · `--mode session --route real-login --plant`. Finding: real %LOCALAPPDATA%jevjev.log holds 4 empty-payload refusals from a jev test with no LOCALAPPDATA isolation. Lesson: "delivered to the context" and "the model says it saw it" are two observations — the transcript proved the first, the echo did not prove the second.

## 2026-09-24 (on D) — D130 A: defect 1 fixed — jev-judge now HASHES WHAT IT COMPILED: loadJudgeInputs compiles buildOverseerPrompt from the one read it hashes (no require cache, worker top level never runs), and loadJevModule compiles prompt.js from its hashed bytes (public createRequire + real __dirname) — my own D123 code had the same defect. jev-shadow.js:109 has it too (named, not mine)
`exo_memory/handback/p-d130-hash-A_2026-09-24.md`. Reproduced red 38/4 first; judge 42/0, module 19/0, shadow 32/0, runner 45/0; mutants 7/8 (H2 equivalent: fresh re-read vs compiled string differs only in a race). Lesson: two reads of one file are two facts — hash the string you compile, never a second look at the disk.

## 2026-09-24 (on D) — D132 A: D has not published since 09-10 because NOTHING on the close path publishes — the Leave writes the stick, never the state repo (stick-waiter.js:61, the L059 hold I built); only close.js pushes, by hand. Fixed the carrier: the pulse printed "in sync" over different heads; now "NOT in sync: D behind — last pushed f70d50a 352h ago". A push from D today: 25/64 files, 0 remote rows dropped
`exo_memory/handback/p-d132-publish-A_2026-09-24.md` · preview `exo_memory/loop/d132_publish_preview.js` (read-only; --dry-run writes a receipt). chain-status 110→117/0 unmodified; mutants 7/8 (N7 equivalent). Proposed, not built: close.js at the keeper's close (a keeper call reversing the hold) + a cmdPush divergence gate. Lesson: when three documents believe the same wrong thing, look for the one line everybody reads — here the pulse said "in sync".

## 2026-09-24 (on D) — D133 A: publish at close — LeaveBounds.publish (NORMAL Some(900 s), SHUTDOWN None = never), leave_run step 7 runs close.js AFTER the stick result is written and before the button, and the outcome (PUBLISHED from→to / UNCHANGED / REFUSED verbatim / TIMED_OUT / FAILED / CLOSED) rides beside the result; close.js:319-321 printed "in sync" over ANY heads → syncLine names who is behind. RUST NOT RUN: the heavy lock was held by B for 70+ min, twice
`exo_memory/handback/p-d133-publishclose-A_2026-09-24.md`. close 36/0 (red on old 30/6); mutants 6/6 (C5 survived the unit-only set, which is why the e2e exists). E owes leave.js rendering of p.publish. LIVE ONLY AFTER A REBUILD. Lesson: a pipe to grep/tail eats the exit code; "exit 0" from a background task is the pipe's, not cargo's — write the status into the log.

## 2026-09-25 (on D) — D137 A: the "3 h delivery stall" was not in the door path — the librarian's AskUserQuestion was open 02:04:45Z→05:32:16Z (keeper answered 23:32 local), the call_chair was MADE at 05:32:48.184Z and rendered in Main 0.22 s later; the "20:1x" was hand-made. Pinned anyway: ring_reply(dest, receipt) — "delivered" only from Receipt::Received — used by both rings; 3 tests, cargo 958/0/4 (first run of D133's 11), mutants 5/5
`exo_memory/handback/p-d137-stall-A_2026-09-24.md`. Real defect named for the chair: a keeper question hidden in a seat's pane surfaces only as "keep-warm MISSED … a turn runs". Tool finding: cargo not on PATH here — heavy-cargo spawned ENOENT. Lesson: before tracing why a delivery was late, find when it was SENT — the sender's transcript tool_use timestamp, not the sender's prose about it.

## 2026-09-25 (on D) — D140 A: the About tab is now README.md's about block word for word (static, between the same markers), pinned by consonance/ui/about-readme.test.js (5/0; red first 2/3; mutants 10/10, CRLF + re-wrap controls 3/3 green); links render as words (no opener: an href would navigate the app away). Kept "The loop" (lap-row.test.js:1055 pins it) and the hero — R3 questions. History file moved to loop/readme_history_2026-09-25.md, sha256 bee9bee8… unchanged; carrier-drift 57/0; arch_test 12/1 — README.md:228 (C's) still links the old path, and it needs the file tracked
`exo_memory/handback/p-d140-about-A_2026-09-25.md`. js-suite 140 green/0 failed of 142. Lesson: before replacing a section wholesale, grep the tests for what reads it — a pinned paste inside the section belongs to another seat's test.

## 2026-09-25 (on D) — D142 A: About re-rendered from C's revised block (4 sections; the loop diagram gone, per the keeper), header = tagline + the README lead line, now pinned too; lap-row's About-diagram pin deleted (that one test only, 135/0); .aboutlink dotted. about-readme 6/0 (red on pre-D142 4/2); mutants 13/13, controls 3/3; carrier-drift 57/0; arch_test 12/1 = the two untracked link targets (history file + my test), green only when they land with README
`exo_memory/handback/p-d142-about-A_2026-09-25.md`. Lesson: mutants anchored on prose go NOT APPLIED when the prose is revised — re-point them before counting, or the count silently shrinks.

## 2026-09-25 (on D) — D143 A: dead panes — REUSED the one waiter thread (it already emitted pty-exit); new pane_exit.rs decides: an exit counts only if the map still holds THIS pid and no Leave; fixed seats reopened ONCE per run with --resume (a second exit is posted, not reopened); committee panes prompted with ↻; keep-warm skips an exited child and its row says EXITED, not D138's "waiting on YOUR answer". The garbled reopen: term.reset + clear sentRows/sentCols BEFORE the new process, for ↻ and the auto path both. cargo 973/0/4 (red 0/7); Rust mutants 12/12 (4 by source shape only); JS 6/0, mutants 9/9
`exo_memory/handback/p-d143-deadpane-A_2026-09-25.md`. One existing test's text anchor re-pointed (leave_wiring `child.wait()`), named. NOT live until rebuild. Lesson: before building a detector, look for the one that already fires — here detection existed and only its consequences were missing.
