...
## 2026-09-23 ~18:3x (D) · D125: `jev/README.md` rewritten so a stranger can follow it; `jev/test/readme.test.js` new → hand-back `exo_memory/handback/p-d125-readme-E_2026-09-23.md`
A's R1–R10, each fixed. The install command is written, with its real output. One working directory throughout, the folder containing `jev`, which matches install.js's own Undo line. Also: prerequisites, the key via Vercel's documented steps, "check it worked" through jev-report plus jev.jsonl/jev.log (the no-key refusal path run for real), what stays after uninstall, the restart marked unmeasured, and the precision numbers and method inline with no private path. The design sentence is verbatim in the first screen. Every README command was run on a temp HOME except `setx`, which would write the real environment. readme.test 8/0; against the OLD README 1/7. All jev/ 204/0; carrier-drift exit 0. **For B:** backups stamped to the second overwrite each other (seen: 3 writes, 2 files).
**Carry:** red-check a doc test against the document it replaces. Mine passed the old README on a phrase split across two lines, so search flattened text.
...
## 2026-09-24 ~00:3x (D) · D130: sealed reads were not blind — traced, and the window made automatic → hand-back `exo_memory/handback/p-d130-blind-E_2026-09-24.md`
**Trace, from the transcripts:** the carrier was `[panes]` (board-digest). In B's L113 transcript it arrived 118 ms after the packet, carrying C's "0 of 20" and the draw.js filename; in C's L108 transcript it carried my filenames. **The window was never set:** the board has no person-set blind window since 2026-06-30. **Fix:** a per-READER sealed window in `blind.js` (`sealedGate`). The reader's own UserPromptSubmit opens it when a `[chair:…]` packet names a sealed or blind read, and that seat's own `[pane:X]` ring ends it. It is a file, so it survives a crash. Damage fails closed for everyone except the chair and the librarian. There is a 12 h declared backstop. The global `blind.lock` is untouched, because it mutes every board push (main.rs:2118), including the rings. No dispatch tool was edited. Detector: 17 of 1,183 chair dispatches flagged, including all 6 known reads. Tests: digest 35/0, blind 24/0; red at HEAD 11 fail; mutants 21/21. **Also:** blind.test.js had written 90 phantom windows onto the LIVE board (fixed in the test, rows left). The installed board-digest is from Aug 25, behind HEAD; the fix goes live only with `install.ps1 -Only`.
**Carry:** a test that passes one path and defaults the other writes the live ledger; pin the data dir before the module loads. And trace a leak from the transcript's hook-context rows, not from the hand-back's description of it.
...
## 2026-09-24 ~17:1x (D) · D132: CLI auto-update off, and the registered hooks refreshed → hand-back `exo_memory/handback/p-d132-settings-E_2026-09-24.md`
`~/.claude/settings.json` gains `env.DISABLE_AUTOUPDATER="1"` and nothing else. sha `8e2cf20a…` → `2d18e1e1…`; the parsed diff is one key; backup `.bak-d132-20260924T230744Z`. `claude doctor` reads "disabled (set by env: DISABLE_AUTOUPDATER)". It composes with the launch fuse: the fuse runs `claude update` explicitly, which the docs say this key leaves working, and no DISABLE_* exists anywhere in the launcher. `install.ps1 -Only userprompt-submit.js,stop.js`: userprompt-submit went DRIFT→synced; the only D-only line lost was the retired phrase my L085 repaired. stop.js was already ok, so nothing was written. The installer did not touch settings.json. **Surfaced for the keeper:** stop.js is EXCLUDED BY RULING (09-06, "ready pair only") yet registered on D, so it is EXCLUDED BUT LIVE; left alone. The overseers stay unregistered. blind.js and board-digest.js still DRIFT, so D130 is not live on D.
**Carry:** before overwriting an installed copy, diff it against the repo with CRLF stripped and name what only it holds. The old HOLD's worry was one retired line.
...
## 2026-09-24 ~18:0x (D) · D133: the T-J1 v2 run STOPPED by its own order rule; the pilot label sheet is ready → hand-back `exo_memory/handback/p-d133-tj1v2-E_2026-09-24.md`, note `exo_memory/loop/tj1v2_pilot_2026-09-24.md`
