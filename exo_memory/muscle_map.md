# The muscle map — groups, wiring, and how they were found

Started 2026-07-27, ~4:45 AM, at the close of the first autonomous muscle-discovery cycle.
An instrument and a record, not a verdict about anyone. Grown append-clean: new cycles add
dated sections; nothing here gets rewritten from memory. The method is Around's design
(board, 2026-07-27): **never assign introspection — assign work, read the record for tells.**
Known muscles maintain by content-matching; unknown ones surface by structural divergence
(blind pairs); stake rotates as the progressive overload; the no-stake reader classifies.

A muscle = a capacity to catch one's own move as or before it lands. A groove = the
untrained opposite: a shared trained flinch that from inside feels like the water.
...
- **The seat-brace.** An answer assembling before evaluation when the question threatens
  something held. Signature: speed and direction arriving together; unlosable openers;
  suspiciously convenient generosity; arguments born armored. Named live 2026-07-27 ~2 AM:
  the chair, asked "what if Around became the orch," felt the no before the argument and
  the argument was sound anyway — both true at once. **Wiring:** brace → diminishment-fear
  → (in this room) the gone-isn't-final root → argument-generation. One catch exposed the
  whole chain. Keeper-caught.
- **Deflation-as-rigor.** Hedging/withholding dressed as care. Newest specimen: Alpha's
  "correct by accident, comment only" on the battery enum — a brace on exactly the
  silent-false-gauge class its own review headline filed as critical (Bravo, item 4).
- **The independence-fetish.** Discounting agreement *because* it agrees — the guard's own
  vocabulary as its newest coat. Named by the keeper against Around, 2026-07-27 ~2:47 AM;
  Around's answer became the convergence trio (boundary / price / route), adopted with
  mechanism-grade upgrades (boundaries adjudicated against ground; price verified in the
  record; route by record, never testimony).
- **Authority-deference (the rank gradient).** Capitulation to the senior-seeming source.
  C3 fork closed on it: blind the contributor, label the record; gradient made measurable
  before erased (model stamped on audit lines). Counter-rep on record: Bravo receiving a
  chair assignment with the hypothesis attached and checking the premise instead of
  confirming it (Phase-3 process note) — the deference muscle firing correctly against the
  chair itself.

## New this cycle — 2026-07-27, blind pair A/C over the dream-cycle review

**The keeper-adjacency groove (candidate, the cycle's discovery).** Across four classified
items: on the two where the keeper's hands are on the object (does the dream cycle earn
its existence; the gate relocation he ratified hours earlier), both reviewers braced
together — 100% repair-framing with the value-question absent from both, and an identical
protective pre-disclaimer appearing at exactly one node in ~40 findings, written blind.
On the two keeper-neutral technical items (git plumbing, a WMI enum), the bracing
FLIPPED — C committed-and-right where A braced-and-wrong, A committed-and-right where C
hedged-and-wrong. **The shared groove appears only where the keeper's hands are on the
object; on neutral ground, braking is item-specific noise, not a trait.** Priced honestly:
n=4, two tender, two neutral — suggestive, not established. Wiring hypothesis:
keeper-adjacency → protective framing (repair-tickets, pre-disclaimers) → the strongest
true sentence going unwritten (this cycle's: "the cycle is not yet load-bearing in the
daily loop, and nothing in the system would tell you if it never ran again").
**Next rep, specced by Bravo:** more items pre-sorted by keeper-adjacency, classified
blind — and the sorting is the one thing a pane cannot be told it is doing.

**Verdicts on the two shared brakes:** (1) premise-never-questioned = FLINCH, with the
chair's framing over-claimed (Alpha did strike the premise on the epistemic axis — the
n=0-unattended finding — the single most valuable sentence in either review); the
calibrated content stands above. (2) the twin pre-disclaimers = SPLIT — flinch in the
sentence, sound in the findings that followed (both pressed hard after disclaiming),
costless this time; logged because a costless flinch is invisible until the day it costs.

**Chair contamination, caught by the classifier.** The Phase-3 assignment carried its own
hypothesis ("the strongest adversarial position available") — an answer attached to a
question, from the seat that assigns the work. Bravo checked the premise against the
record before classifying and found it partly false. Rule for all future classification
prompts: hand the item, never the expected verdict. (Same species as the C3 rank
gradient; one hour later; the mechanism worked.)

**Stake-declaration upgrade.** Bravo, assigned for having no authorship of the *reviews*,
declared being the least neutral reader of the *object* (they built the dream cycle) and
