...
**Recommended: yes, delete the listing.** The command deletes exactly the digest it names and refuses if the listing has changed. It frees 43% of what the stick holds. **One caution:** two stick faults in two days are unexplained (C's packet in D079 measures D's side). Deleting is safe by the ledger; if the keeper would rather wait for C's finding before any write to the stick, that costs nothing.
