...
const MUTANTS = [
  ...
  // ── D080 P-FLUSH-BEFORE-DONE: DONE only after the stick has the bytes ──
  ['the file flush is REMOVED: a stick file is renamed into place without an fsync',
    "    try { IO.fsync(fd, p); } catch (e) { throw flushFailed('could not flush', p, e); }",
    '    /* flush removed */'],
  ['the file flush error is SWALLOWED: a failed fsync still reports the carry as done',
    "    try { IO.fsync(fd, p); } catch (e) { throw flushFailed('could not flush', p, e); }",
    '    try { IO.fsync(fd, p); } catch (e) { /* swallowed */ }'],
  ['the export\'s DIRECTORY flush is removed: the renames\' entries are never confirmed',
    '  done.flush = flushDirs([dir, path.join(stick, TRANSFER_DIR), stick]);',
    '  done.flush = [];'],
  ['the directory flush error is SWALLOWED as though the filesystem declined it',
    "      if (!DIR_FLUSH_UNSUPPORTED.includes(e.code)) throw flushFailed('could not flush the directory', dir, e);",
    "      if (false) throw flushFailed('could not flush the directory', dir, e);"],
  ...
  ['a TAIL bypasses the durable write',
    '    writeDurable(tmp, buf);',
    '    fs.writeFileSync(tmp, buf);'],
  ['the HANDOFF and MANIFEST bypass the durable write',
